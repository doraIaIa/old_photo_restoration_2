...
| Registry metric đã ghi nhận best metric của smoke run | `results/registry/metric_registry.csv` | csv | Best validation IoU/F1/Precision/Recall của từng run |
...
| Prediction export của smoke run r003 cho thấy ảnh hưởng của Dice-heavy loss | `outputs/debug/segmentation/seg-unet-attn-r003-dice07-s42/` | image panels | So sánh trực quan với r002 trên cùng val split |
| Threshold sweep của smoke run r003 cho thấy calibration khác r002 | `experiments/segmenter/seg-unet-attn-r003-dice07-s42/threshold_sweep.csv` | csv | Ngưỡng tối ưu theo IoU/F1 quay về vùng thấp hơn |
| Prediction export của smoke run r004 cho thấy ảnh hưởng của việc tăng base channels | `outputs/debug/segmentation/seg-unet-attn-r004-ch32-s42/` | image panels | So sánh trực quan với model nhỏ hơn |
| Threshold sweep của smoke run r004 cho thấy model lớn hơn chưa thắng r002 | `experiments/segmenter/seg-unet-attn-r004-ch32-s42/threshold_sweep.csv` | csv | Capacity tăng không tự động cải thiện IoU tốt nhất |
| Prediction export của smoke run r005 cho thấy baseline dài hơn cải thiện coverage của crack | `outputs/debug/segmentation/seg-unet-attn-r005-long-s42/` | image panels | Vẫn còn vài false positive nhỏ nhưng mask bám crack tốt hơn rõ rệt |
| Threshold sweep của smoke run r005 cho thấy threshold tối ưu dịch xuống vùng 0.25–0.30 | `experiments/segmenter/seg-unet-attn-r005-long-s42/threshold_sweep.csv` | csv | Best IoU/F1 nhỉnh hơn hẳn r002 trên cùng dataset |
